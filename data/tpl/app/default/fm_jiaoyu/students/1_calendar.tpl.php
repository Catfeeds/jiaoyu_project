...
<section>
    <div class="conentBox">
        ...
        <div class="top_bottom"><?php  echo $student['s_name'];?></div>
    </div>
</section>
	...
	if (checkClick(e, $(this))) {
		if ($(this).find('.dateBox').hasClass('weekBox') && $(this).find('.tipBox') == '') {
			...
			switch (sType) {
				case "签":
					$.ajax({
						url: "<?php  echo $this->createMobileUrl('dongtaiajax', array('op' => 'checklogbyid','bj_id' => $student['bj_id'],'schoolid' => $schoolid), true)?>",
						type: "post",
						dataType: "html",
						data: {
							time: time,
							...
							for (var i = 0; i < dt.length; i++) {
								...
								if (type == "card") {
									if (!url2){
										...
										alertHtml += "<div class=\"schoolCard\">体温:" + dt[i].Temp + "</div>";
										alertHtml += "<div class=\"schoolCardTime\">时间：" + dt[i].Time + dt[i].Macname + "</div>";
										...
										alertHtml += "<div class=\"schoolCard\">体温:" + dt[i].Temp + "</div>";
										alertHtml += "<div class=\"schoolCardTime\">时间：" + dt[i].Time + dt[i].Macname + "</div>";
										alertHtml += "<div class=\"schoolCardCardImg\"><img src=\"" + url + "\" class=\"img-responsive\"></div></br>";
...
switch (type) {
	...
	case "我要签到":
		if ("True" == "True") {
			if ("True" == "True") {
				window.location.href = "<?php  echo $this->createMobileUrl('wxsign', array('schoolid' => $schoolid,'userid' => $it['id']), true)?>";
			} else {
				jTips("未开启签到功能！");
			}
		} else {
		...
		break;
	case "我要请假":
		window.location.href = "<?php  echo $this->createMobileUrl('xsqj', array('schoolid' => $schoolid,'userid' => $it['id']), true)?>";
		break;
	case "请假记录":
		window.location.href = "<?php  echo $this->createMobileUrl('leavelist', array('schoolid' => $schoolid,'userid' => $it['id']), true)?>";
		break;
	case "考勤卡":
		window.location.href = "<?php  echo $this->createMobileUrl('checkcard', array('schoolid' => $schoolid,'userid' => $it['id']), true)?>";
		break;		
	default:
		break;
}
