...
<div class="top_head_blank" style="height:50px" id="titlebar_bg"></div>
        ...
        <dl id="main">
            ...
            <dd class="count_inf">
                ...
                <ul>
					...
					<?php  if($item['isSign'] == 1 ) { ?>
                    <a class="outline_link" onclick="startsign()" id="kcsign">签到</a>
					<?php  } ?>
                    </li>
                    <li><label>科目：</label><?php  echo $category[$item['km_id']]['sname'];?></li>
                    <li><label>教室：</label><?php  echo $category[$item['adrr']]['sname'];?></li>
                    <li><label>授课教师：</label><?php  echo $tname_array_end;?><?php  echo $item['ReNum'];?></li>
                    <?php  if($item['OldOrNew'] == 1 || $item['ReNum'] != 0) { ?>
                    <li><label>总课时/已购课时：</label><?php  echo $item['AllNum'];?>/<?php  if(!empty($ygks)) { ?><?php  echo $ygks['ksnum'];?><?php  } else { ?>0<?php  } ?>课时<a class="outline_link" onclick="showxgks()" id="xgks">续购</a></li>
                    <?php  } ?>
                </ul>            
                <div style="clear:both"></div>
            </dd> 	
        </dl>
		<?php  if(GetSchoolType($schoolid,$weid) == 1) { ?>
		<?php  if($share != 0) { ?>
		<dl class="count_outline">
            <dt><a href="<?php  echo $this->createMobileUrl('mysharedetail', array('id' => $it['id'],'schoolid' => $schoolid,'kcid'=>$item['id']), true)?>" >已成功邀请<span class="outline_link" style="float:unset;margin-right:3px;margin-left:3px;" ><?php  echo $share;?></span>人 <span class="outline_link" >查看更多</span></a></dt>
        </dl>
		<?php  } else { ?>
	...
	<?php  if($item['OldOrNew'] == 0 && $item['isSign'] == 1) { ?>
	<div id="wrap" class="user_inf">
        ...
        <dl class="counts">
			...
			<?php  if(is_array($list)) { foreach($list as $item1) { ?>
               <?php  if($item1['date'] > TIMESTAMP) { ?>
               <dd>
               <?php  } else { ?>
                	<?php  if($item1['checksign'] ==1) { ?>
                    <dd class="click1" onclick="touch(<?php  echo $item1['id'];?>)" >
                	<?php  } else if($item1['checksign'] ==2) { ?> 
                    <dd class="click" onclick="touch(<?php  echo $item1['id'];?>)" >
                	<?php  } else { ?>
                    <dd class="click" style="background-color: #9cd0c8;" onclick="touch(<?php  echo $item1['id'];?>)" >
                   <?php  } ?>
               <?php  } ?>
               第<span><?php  echo $item1['nub'];?></span>课<br/><?php  echo date('m月d',$item1['date'])?></br><?php  if(!empty($category[$item1['sd_id']])) { ?><?php  echo $category[$item1['sd_id']]['sname'];?><?php  } ?>
               </dd>          
			<?php  } } ?>
            	<div style="clear:both"></div>
            </div>
    ...
    <?php  } else if($item['OldOrNew'] == 0 && $item['isSign'] ==0) { ?>
    <div id="wrap" class="user_inf">
   	...
   	<div id="wrap" class="count_inf">
        <dl class="counts">
            ...
            <div>
			<?php  if(is_array($list)) { foreach($list as $item1) { ?>
                      <dd<?php  if($item1['isxiangqing'] == 1) { ?><?php  if($item1['date'] > TIMESTAMP) { ?><?php  } else { ?> class="click1" onclick="touch(<?php  echo $item1['id'];?>)"<?php  } ?><?php  } else { ?><?php  if($item1['date'] > TIMESTAMP) { ?><?php  } else { ?> class="click" onclick="touch(<?php  echo $item1['id'];?>)"<?php  } ?><?php  } ?>>第<span><?php  echo $item1['nub'];?></span>课<br/><?php  echo date('m月d',$item1['date'])?></br><?php  if(!empty($category[$item1['sd_id']])) { ?><?php  echo $category[$item1['sd_id']]['sname'];?><?php  } ?></dd>          
			<?php  } } ?>
                      <div style="clear:both"></div>
            </div>
        </dl>
	...
	<div id="wrap" class="count_inf">
        <dl class="counts">
            <dt>学习记录</dt>
            <div>
			<?php  if(is_array($signlist)) { foreach($signlist as $key => $item1) { ?>
			<li style="font-size: 13px">第<span style="color:red;font-weight:bold"><?php  echo $key+1?></span>次 <?php  if($item1['status'] ==1) { ?><span style="color:blue">【未确认】</span><?php  } else { ?><span style="color:green">【已确认】</span><?php  } ?></li>
			...
			<div class="component-dialog-body">
				<form class="form-order" novalidate="novalidate">
					...
					<div class="form-line is_show" style="display: none">
						<div class="input-wrapper" style="border:none;">
							<span>抵用积分:</span>
							<input type="number" placeholder="请输入抵用积分" id="PointNum" name="PointNum" min="<?php  echo $item['MinPoint'];?>" max="<?php  echo $item['MaxPoint'];?>" required="">
						</div>
					</div>
					<?php  } ?>
					<div class="form-line">
		...
		<?php  } else { ?>
			if(OldOrNew == 1){
				...
				<?php  if(!empty($isHaveKs)) { ?>
					//有课时
					var sign_timeRange = <?php  echo $item['signTime'];?> * 60 ;
					if(sign_timeRange != 0){
