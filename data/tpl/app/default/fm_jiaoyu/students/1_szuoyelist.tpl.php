...
<div id="titlebar" class="header mainColor">
	...
	<div class="m"><a><span style="font-size: 18px">作业列表</span></a></div>
	<div class="r">
	</div>
</div>
...
// 底部加载更多
	new Scroll_load({
		...
		"ajax_url": "<?php  echo $this->createMobileUrl('szuoyelist', array('schoolid' => $schoolid, 'bj_id' => $bj_id), true)?>",
	   ...
	   strConvertHtml();
		...
		$('body').on('click',  '.main', function(e) {
			if (!$(e.target).parent().is('.notifyImgItem')) {
				...
				window.location.href = "<?php  echo $this->createMobileUrl('szuoye', array('schoolid' => $schoolid))?>"+ '&id=' + notifyUid;
			} 
		});
