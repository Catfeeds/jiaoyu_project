...
<div class="feedback_box">
        ...
        <select class="feedback_title" id="bj_id">
			<?php  if(is_array($bjlists)) { foreach($bjlists as $row) { ?>
				...
				<?php  $bjlists[] = $row['bj_id'];?>
					<option value="<?php  echo $row['bj_id'];?>"><?php  echo $row['bjname'];?></option>
				<?php  } ?>
			<?php  } } ?>	
        </select>
    </div>
	...
	<div class="feedback_title_box">
		<div class="textBox">
			<input style="border:none" id="title" type="text" placeholder="作业标题...">
        </div>
	</div>	
    <div class="blank"></div>
    ...
    <div class="blank"></div>
     <div class="weui_cell_ft" style="text-align: left;">
    	...
    	<span class="answer_type" hidden>
		   <span class="checkBoxOptionsIco" vstyle="margin-left: 5px;">
...
<script type="text/javascript">
		...
		$.ajax({
			...
			success: function (result) {
				...
				if (result.status == 1) {
					...
					self.hideLoadingMsg();
					ajax_stop_loading();					
					upload_pro(page,noticeid,total, this);
				} else {
	...
	function upload_pro(page, noticeid,total, self) {
		var prodata = {
			weid:"<?php  echo $weid;?>",
			schoolid : "<?php  echo $schoolid;?>",
