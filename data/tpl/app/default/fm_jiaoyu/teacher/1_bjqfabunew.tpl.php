			...
			<?php  if(is_njzr($teachers['id'])) { ?>
				<?php  if($bjlists) { ?>
					<?php  if(is_array($bjlists)) { foreach($bjlists as $row) { ?>
						<?php  if(!in_array($row['bj_id'], $bjlistssss)) { ?>
						<?php  $bjlistssss[] = $row['bj_id'];?>
							<option value="<?php  echo $row['bj_id'];?>" <?php  if($op == 'fxzy' && $row['bj_id'] == $bj_id) { ?> selected = "selected" <?php  } ?>>@<?php  echo $row['bjname'];?></option>
						<?php  } ?>
					<?php  } } ?>		
				<?php  } ?>	
					<?php  if(is_array($mynjlist)) { foreach($mynjlist as $row) { ?>
						<?php  if($bjlists) { ?>
							<?php  if(is_array($row['bjlist'])) { foreach($row['bjlist'] as $item) { ?>
								<?php  if(!in_array($item['sid'], $bjlistssss)) { ?>
...
<div class="add_link_box" <?php  if($op == 'fxzy' ) { ?> style="display: block;"<?php  } ?> style="top:0px">
    <div class="add_link_info_wrap">
        <div class="my_add_link_inner">
            <h3>添加链接</h3>
            <div class="add_link_wrap">
                <span class="my_link_title">链接标题：</span><input class="my_add_link_txt" type="text" id="link_title"  <?php  if($op == 'fxzy' ) { ?> value="<?php  echo $student['s_name'];?>对<?php  echo $leave['title'];?>的回答" <?php  } ?>>
            </div>
            <div class="add_link_wrap">
                <span class="my_link_title">链接地址：</span><input class="my_add_link_txt" type="text" id="link_address" <?php  if($op == 'fxzy' ) { ?> value="index.php?i=<?php  echo $weid;?>&c=entry&schoolid=<?php  echo $schoolid;?>&noticeid=<?php  echo $noticeid;?>&sid=<?php  echo $sid;?>&bj_id44=<?php  echo $bj_id;?>&do=showhomework&m=fm_jiaoyu " readonly <?php  } ?>>
            </div>
