		...
		<?php  $bjlistsss = array() ;?>
			<?php  if($bjlists) { ?>
				<?php  if(is_array($bjlists)) { foreach($bjlists as $row) { ?>
					<?php  if(!in_array($row['bj_id'], $bjlistsss)) { ?>
					<?php  $bjlistsss[] = $row['bj_id'];?>
						<li <?php  if($bj_id == $row['bj_id']) { ?>class="act"<?php  } ?> classid="<?php  echo $row['bj_id'];?>"><div><?php  echo $row['bjname'];?></div></li>
					<?php  } ?>
				<?php  } } ?>	
			<?php  } ?>	
			...
			<?php  if($bjlists) { ?>
				<?php  if(is_array($row['bjlist'])) { foreach($row['bjlist'] as $item) { ?>
					<?php  if(!in_array($item['sid'], $bjlistsss)) { ?>
						<li <?php  if($bj_id == $item['sid']) { ?>class="act"<?php  } ?> classid="<?php  echo $item['sid'];?>"><div><?php  echo $item['sname'];?></div></li>
					<?php  } ?>
				<?php  } } ?>
			<?php  } else { ?>
				<?php  if(is_array($row['bjlist'])) { foreach($row['bjlist'] as $item) { ?>
		...
		<?php  } else { ?>			
			<?php  if($teachers['status'] == 2) { ?>
				<?php  if(is_array($bjlist)) { foreach($bjlist as $row) { ?>
					<li <?php  if($bj_id == $row['sid']) { ?>class="act"<?php  } ?> classid="<?php  echo $row['sid'];?>"><div><?php  echo $row['sname'];?></div></li>
				<?php  } } ?>
			<?php  } else { ?>
				<?php  if(is_array($bjlists)) { foreach($bjlists as $row) { ?>
					<?php  if(!in_array($row['bj_id'], $bjlistsss)) { ?>
...
<div class="slide_left_menu_bg" id="schoollist">
    <div class="slide_left_menu">
		...
		<?php  if(is_array($schoollist)) { foreach($schoollist as $row) { ?>
			<li onclick="isSelects(<?php  echo $row['schoolid'];?>);" <?php  if($schoolid == $row['schoolid']) { ?>class="act"<?php  } ?>>
				<div class="user_img">
...
<div class="top_head_box3 index_top ">
    <div class="index_img">
        <a><img src="<?php  if($teachers['thumb']) { ?><?php  echo tomedia($teachers['thumb'])?><?php  } else { ?><?php  echo tomedia($school['tpic'])?> <?php  } ?>" class="studentImgError"></a>
    </div>
    <!--个人信息的积分和姓名和等级-->
    <div class="index_username1">
		<?php  if($mallsetinfo['isShow'] == 1 ) { ?>
		<div class="jifenImg"></div>
		<div class="jifen">积分:<?php  echo $teachers['point'];?></div>
		<?php  } ?>
        <span class="baobaoName"><?php  echo $teachers['tname'];?></span>
...
<div class="myCrowns">
	...
	<div class="_myCrowns">
		<div class="crowns">
			<div class="mydiv" <?php  if($bzj) { ?>onclick="showlist(1);"<?php  } ?>>
		...
		<div class="crowns2">
			<div class="mydiv" <?php  if($bzj) { ?>onclick="showlist(2);"<?php  } ?>>
		...
		<div class="crowns3">
			<div class="mydiv" <?php  if($bzj) { ?>onclick="showlist(3);"<?php  } ?>>
		...
		<div class="crowns4">
			<div class="mydiv" <?php  if($bzj) { ?>onclick="showlist(4);"<?php  } ?>>
	...
	<li time="<?php  echo date('Y-m-d H:i:s', $item['createtime'])?>">
        ...
        <div class="user_content" style="padding-bottom:10px;">
			...
			<ul class="user_img_list3">
				...
				<li class="no_image_tag3" style="height: 275.306px;">
					<div class="li_video3" video_url="<?php  echo tomedia($item['video'])?>" isreport="N" style="background-image:url(<?php echo OSSURL;?>public/mobile/img/videoicon.png);">
						<div class="icon"></div>
					</div>					
				</li>					
				<?php  } ?>
...
<div class="selectList" style="display: none; margin-top: -217.5px;">
	<div class="double" id="classList" style="display: block; height: 436px;">
		<ul style="height: 386px;">
			<?php  if(is_array($thisclassstu)) { foreach($thisclassstu as $row) { ?>
				<li onclick="isSelect(this);">
					<span name="checkName" class="le"><?php  echo $row['s_name'];?> </span>
	...
	$(function () {
		...
		function changePraiseIco($praisBox, $currentObj) {
			...
			if (img_url.toString().indexOf("icon_nopraise") > 0) {
				...
				$praisBox.append('<span class="praiseContent" user_id="<?php  echo $fan['uid'];?>"><?php  echo $teachers['tname'];?></span>');
			}
		}
			...
			if (comment_sumit_tag) {
				...
				$.ajax({
					...
					data: {
						...
						uid: "<?php  echo $fan['uid'];?>",
						hftoname: reply_user,
						shername: "<?php  echo $teachers['tname'];?>老师",
						openid:"<?php  echo $openid;?>",
					},
					success: function (data) {

						if (common_type == "comment_reply") {
							parent_div.find(".comment_list3").show().append('<li diary_id="' + diary_id + '" reply_user="<?php  echo $teachers['tname'];?>老师" comment_id="' + data.id + '" is_mine="true" type="comment_reply"><div class="comment_content"><div class="text"><span class="user_name"><?php  echo $teachers['tname'];?>老师</span>回复<span class="user_name">' + reply_user + "</span><span>：</span>" + comment_context_change + "</div></div></li>");
						} else {
							parent_div.find(".comment_list3").show().append('<li diary_id="' + diary_id + '" reply_user="<?php  echo $teachers['tname'];?>老师" comment_id="' + data.id + '" is_mine="true" type="comment_reply"><div class="comment_content"><div class="text"><span class="user_name"><?php  echo $teachers['tname'];?>老师</span><span>：</span>' + comment_context_change + '</div></div></li>');
						}
		...
		$(".new_diary_list3").on("click", ".other_control_icon_praise", function (e) {
			...
			$.ajax({
				...
				data: {
					...
					userid: "<?php  echo $it['id'];?>",
					uid: "<?php  echo $fan['uid'];?>",
					zname: "<?php  echo $teachers['tname'];?>老师"					
				},
				success: function (data) {
					if (data.result) {
