...
<div class="clear"></div>	
	...
	<div class="popUpBox" style="display: none">
		...
		<div class="popContentBox">
			<section class="sectionContBox">
				<input type="hidden" id="notifyID" value="" />
				<input type="text" id="notifyType" class="poptitle" value="" />
				<div class="popNotifyDescribe">
					<div id="notifyContent" contenteditable="true"></div>
					<ul class="baby_diary_img_list baby_diary_img_listOther"></ul>
				</div>
				<div class="btnBox">
					<div class="btnPass">提交</div>
					<div class="btnCancel">取消</div>
				</div>
			</section>
		</div>
	</div>
    <div class="F_div qx_00302" onclick="addnote();">
        <div class="F_div_text">发作业</div>
    </div>
...
setTimeout(function() {
	...
	function isSelect(bjid){
		jTips("数据加载中！···");
		location.href = "<?php  echo $this->createMobileUrl('zuoyelist', array('schoolid' => $schoolid), true)?>"+ '&bj_id=' + bjid;
	}
	...
	function addnote(){
		location.href = "<?php  echo $this->createMobileUrl('zfabu', array('schoolid' => $schoolid), true)?>";
	}
...
//deleteImg
	...
	$('.btnPass').click(function(e) {
		...
		var notifyContent = $("#notifyContent").html();
		var notifyStatus = $("#notifyStatus").val();

		var str = arrayImg;
		var imgList = str.join(',');

		var ajaxData = {};
		ajaxData.NotifyID = notifyID;
		ajaxData.Type = notifyType;
		ajaxData.Content = notifyContent;
		ajaxData.ImgList = imgList;

		$.ajax({
			type: "POST",
			url: "/1046/Notify/GetNotifyAuditThroughEdit",
			dataType: "json",
			data: ajaxData,
			success: function(msg) {
				if (msg.status == "1") {
					jTips(msg.info, function() {
						clear_page_session('teacher_leader_notify');
						window.location.href = "/1046/Notify";
...
// 底部加载更多
	new Scroll_load({
		...
		"ajax_url": "<?php  echo $this->createMobileUrl('zuoyelist', array('schoolid' => $schoolid, 'bj_id' => $bj_id), true)?>",
		...
		"after_ajax": function () {
			...
			strConvertHtml();
			<?php  if(!(IsHasQx($tid_global,2000302,2,$schoolid))) { ?>
				$(".qx_00302").hide();
			<?php  } ?>
			<?php  if(!(IsHasQx($tid_global,2000303,2,$schoolid))) { ?>
				$(".qx_00303").hide();
			<?php  } ?>
		}
	}).load_init();
...
/*删除通知老师端*/
	function del_notice(id, e) {
		e = window.event;
		e.stopPropagation();
		jConfirm('确认删除此通知吗？', '', function(r) {
			if (r) {
				$.ajax({
					url: "<?php  echo $this->createMobileUrl('dongtaiajax', array('op' => 'delnotice'), true)?>",
					type: 'POST',
					dataType: 'json',
					data: {
						sNotifyUid: id
					...
					success: function(result) {
						jTips(result.info, function() {
							if (result.status == 1) {
								$('li[id="' + id + '"]').remove();
								clear_page_session('teacher_notify');
								//location.reload();
							}
						});
					}
	   ...
	   strConvertHtml();
		...
		$('body').on('click',  '.main', function(e) {
			if (!$(e.target).parent().is('.notifyImgItem')) {
				...
				window.location.href = "<?php  echo $this->createMobileUrl('zuoye', array('schoolid' => $schoolid))?>"+ '&id=' + notifyUid;
			} 
		});
