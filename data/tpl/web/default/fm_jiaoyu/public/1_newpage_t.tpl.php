...
<!-- <script type="text/javascript" src="../addons/fm_jiaoyu/public/web/js/drag.js"></script> -->
<style>
...
.card:hover > .card-remove,.card:hover > .card-config,.card:hover > .card-refresh {
...
<div id="titlebar" class="header mainColor" style="position: relative;background:<?php  if($logo['headcolor']) { ?> <?php  echo $logo['headcolor'];?><?php  } else { ?> #06c1ae;<?php  } ?>">
	<div class="l"><a class="backOff" style="background:url(<?php echo OSSURL;?>public/mobile/img/ic_arrow_left_48px_white.svg) no-repeat;background-size: 55% 55%;background-position: 50%;" ></a></div>
	<div class="m">
		<span style="font-size: 18px">教师中心</span>   
	</div>
	<div class="r">
		<a class="choice_baby">
            <img style="width:30px;margin-top:10px;height:30px"  src="<?php echo OSSURL;?>public/mobile/img/selectMean.png" class="img-responsive">
        </a>
	</div>
</div>
		...
		<div class="stuBox">
			<div  class="stuInfo">
				...
				<div class="stuInfoCenter">
					<div class="stuName">
						<label class="m_r_10" style="font-size: 16px;font-weight: unset;">教师名称</label>
			...
			<?php  if($logo['is_wxsign'] ==1 || $logo['is_recordmac'] ==1) { ?>
			...
			<div class="stuServer">
				<label>聊天</label>
				<div class="server">
					<span id="msg_word">是否接收家长私聊信息以及公开电话&nbsp;</span>
				</div>
		...
		<div class="linkDiv grid" id="linkDiv" style="margin-top:10px;">
			<?php  if(is_array($iconsF)) { foreach($iconsF as $key => $row) { ?>	
			  <div class="item  w1 red muuri-item muuri-item-shown linkBox"
                 data-id="<?php  echo $row['ssort'];?>" data-color="red" data-title="mk"
                ...
                <div class="item-content " style="opacity: 1; transform: scale(1);">
					...
					<span class="linkName"><?php  echo $row['name'];?></span>	 
                        ...
                        <div class="card-remove"><i class="material-icons fa fa-bars"  onclick="showMoFang(<?php  echo $row['id'];?>)"></i></div>
                    </div>
