			...
			<div class="panel-body" >
				...
				<div class="form-group">
					...
					<div class="col-sm-9 col-xs-6">
						<div class="input-group text-info">
							<label class="checkbox-inline"><input type="checkbox"	name="allchecked"  value="allc" class="check_all" <?php  if($item1['isall']) { ?>checked="checked" <?php  } ?> />全体学生</label>
				...
				<div class="form-group">
							...
							<div class="col-sm-9">
								<label class="radio-inline">
									<input type="radio" name="is_allsend" value="1" <?php  if($reply['is_wxsign']== 1 ) { ?>checked<?php  } ?> id="credit8">是
								</label>
								<label class="radio-inline">
									<input type="radio" name="is_allsend" value="2" <?php  if($reply['is_wxsign'] == 2 || empty($reply['is_wxsign']) ) { ?>checked<?php  } ?> id="credit9">否
								</label>
				...
				<div class="form-group">
					<label class="col-xs-12 col-sm-3 col-md-2 control-label"><span style="color:red">*</span>报名费</label>
                     <div class="col-sm-2 col-lg-2">
				        <div class="input-group">
						<input type="text" name="bmfy" class="form-control" value="<?php  echo $item1['cost'];?>" />
                        </div>
				     </div>
			    </div>	
		...
		<table class="table">
			...
			<tbody>
				...
				<tr>
					...
					<td>
						<?php  if($item['cost'] != 0 ) { ?>
						<span class="label label-success"><?php  echo $item['cost'];?></span>
						<?php  } else { ?>
...
$(function(){
	var e_d = 2 ;
	<?php  if((!(IsHasQx($tid_global,1001702,1,$schoolid)))) { ?>
		$(".qx_1702").hide();
		e_d = e_d - 1 ;
	<?php  } ?>
	<?php  if((!(IsHasQx($tid_global,1001703,1,$schoolid)))) { ?>
		$(".qx_1703").hide();
	<?php  } ?>
	<?php  if((!(IsHasQx($tid_global,1001705,1,$schoolid)))) { ?>
		$(".qx_1705").hide();
		e_d = e_d - 1 ;
	<?php  } ?>
	if(e_d == 0){
