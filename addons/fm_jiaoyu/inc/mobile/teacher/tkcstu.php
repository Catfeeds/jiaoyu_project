 ...
 */
		...
		if($thistime){
			...
			foreach($leave1 as $key =>$row){
				$students = pdo_fetch("SELECT id,s_name,numberid,bj_id,sex,icon FROM " . tablename($this->table_students) . " where weid = '{$weid}' And schoolid = '{$schoolid}' And id = {$row['sid']} ");
				$banji = pdo_fetch("SELECT sname FROM " . tablename($this->table_classify) . " where sid = :sid And schoolid = :schoolid ", array(':schoolid' => $schoolid,':sid' => $students['bj_id']));
