 ...
 */
   ...
   $operation = in_array ( $_GPC ['op'], array ('default', 'CheckNewMsg', 'binding_for_students', 'binding_for_teachers', 'make_code', 'AddGather', 'GetKmList', 'GetAllKm', 'GetKmInfo', 'GetAllData','gasignup','horder','createtodo','DealWithTodo','TodoDeliver','addsk','yy_order','cyy_t_beizhu','reset_stuinfo','get_user_qr','huifu_mail') ) ? $_GPC ['op'] : 'default';
	...
	if ($operation == 'AddGather') {
		...
		$nextpoint = pdo_fetchall("SELECT id,macid,lon,lat,createtime FROM " . tablename($this->table_checklog) . " where macid = {$_GPC['macid']} And lon !='' And id > '{$_GPC['nowid']}' $condition ORDER by createtime ASC LIMIT 0,1");
		$point = $nextpoint[0]['lon'].','.$nextpoint[0]['lat'];
		if($nextpoint){
			$gps1 = "[{$point}]";
		}else{
			$gps1 = "[]";
		...
		$temp['data'] = $gps1.";[];[];[];[];[];[];[];[];[]";
		$temp['info'] = 1;
		$temp['id'] = $nextpoint[0]['id'];
		$temp['dataoldGPS'] = null;
		$temp['classname'] = "六年级一班";
		$temp['stuname'] = "小周";
		$temp['createtime'] = 1;
		...
		if (! $_GPC ['schoolid'] || ! $_GPC ['weid']) {
			...
			if (empty($_GPC['openid'])) {
				...
				if(!empty($_GPC['videoMediaId'])){
					...
					if($getfile['code'] == 0){
						$videoimg = $getimg['basicInfo']['coverUrl'];
						$video = $getfile['basicInfo']['sourceVideoUrl'];
						if($school['savevideoto'] == 2){
