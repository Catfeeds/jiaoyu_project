...
$uniacid = !empty($_GPC['u_uniacid']) ? $_GPC['u_uniacid'] : $_W['uniacid'] ;
...
$dos[] = 'chongzhi';
$do = empty($_GPC['paytype']) ? 'wechat' : $_GPC['paytype'] ;
...
if(!empty($type)) {
	...
	if($type != 'delivery') {
		...
		$coupon_info = pdo_get('coupon', array('uniacid' => $uniacid, 'id' => $coupon_id));
	...
	if($type == 'wechat') {
		...
		header("location: ../payment/wechat/pay.php?i={$uniacid}&auth={$auth}&ps={$sl}");
		exit();
	}
	if($type == 'credit') {
	...
	if($type == 'chongzhi') {
		...
		if(!is_error($site)) {
			...
			$yuE = $_GPC['s_yuE'];
			$New = $yuE - $log['fee'];
			pdo_update('wx_school_students', array('chongzhi' => $New), array('id' => $sid));
			//$student_check = pdo_fetch("SELECT chongzhi FROM ".tablename('wx_school_students')." WHERE id = :id", array(':id' => $sid));
			//message($student_check['chongzhi']);
			$site->weid = $_W['weid'];
			$site->uniacid = $uniacid;
			$site->inMobile = true;
			$method = 'payResult';
	...
	if ($type == 'unionpay') {
		$sl = base64_encode(json_encode($ps));
		$auth = sha1($sl . $uniacid . $_W['config']['setting']['authkey']);
		header("location: ../payment/unionpay/pay.php?i={$uniacid}&auth={$auth}&ps={$sl}");
		exit();
	}
	if ($type == 'baifubao') {
		$sl = base64_encode(json_encode($ps));
		$auth = sha1($sl . $uniacid . $_W['config']['setting']['authkey']);
		header("location: ../payment/baifubao/pay.php?i={$uniacid}&auth={$auth}&ps={$sl}");
		exit();
	}
}
