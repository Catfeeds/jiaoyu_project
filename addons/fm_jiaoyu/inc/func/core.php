...
class Core extends WeModuleSite {
    ...
    public function getNaveMenu($schoolid, $action)
    {
		...
		$tag = 1 ;
		if($_W['isfounder'] || $_W['role'] == 'owner' || strstr($qxarr,'100060') || strstr($qxarr,'100070') || strstr($qxarr,'100080') || strstr($qxarr,'10009') || strstr($qxarr,'100100') || strstr($qxarr,'10011') ){
			$navemenu[$tag] = array(
        ...
        //商城
            $navemenu[$tag] = array(
                'title' => '<icon style="color:#47e0d5;" class="fa fa-shopping-cart"></icon>  商城管理',
				'this' => 'no6'	
            );
		...
		if($sms_set['jfjgtz'] == 1 || !empty($smsset['jfjgtz'])) {
			...
			if($order['type'] == 7 ){					
				...
				if(!empty($smsset['jfjgtz'])){				
					$this->sendtempmsg($smsset['jfjgtz'], $url, $data, '#FF0000', $user['openid']);
				}				
			}else{
				if($order['type'] != 6 ){
