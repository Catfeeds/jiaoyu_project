...
function CheckIsAsnwered($sid,$zyid){
	$all = pdo_fetch("SELECT * FROM " . tablename('wx_school_answers') . " where sid= :sid AND zyid = :zyid  ", array( ':sid' =>$sid, ':zyid' => $zyid));
	if(!empty($all))
	{
		return 1;
	}else{
...
function GetPyContent_c($sid,$zyid){
	$all = pdo_fetchall("SELECT tmid,content,tname FROM " . tablename('wx_school_ans_remark') . " where sid= :sid  AND zyid = :zyid  ", array( ':sid' =>$sid,  ':zyid' => $zyid));
	if(!empty($all))
	{
		foreach( $all as $key => $value )
		{
			$temp[$value['tmid']] = $value['content'];
			$temp['tname'] = $value['tname'];
		}
...
function GetPyContent_p($sid,$zyid){
	$all = pdo_fetch("SELECT content,tname FROM " . tablename('wx_school_ans_remark') . " where sid= :sid  AND zyid = :zyid  ", array( ':sid' =>$sid,  ':zyid' => $zyid));
	if(!empty($all))
	{
